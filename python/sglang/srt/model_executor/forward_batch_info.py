...
class ForwardBatch(ForwardBatchDeepSeekMHAMixin):
    ...
    def get_num_non_padded_tokens(
        ...
        """
        # DP-attention extend/prefill path: extend_seq_lens_cpu records the real
        # per-request extend lengths before prepare_mlp_sync_batch pads tensors
        # to a DP/TP/CP-aligned communication size.
        ...
        # Generic fallback for decode/verify/non-DP paths, and for attn-TP
        # adjustment. In extend mode, fall through here only when no CPU extend
