...
class ModelRunner(ModelRunnerKVCacheMixin):
    ...
    def init_torch_distributed(self):
        ...
        logger.info(
            f"[SpanLogs] Init torch distributed ends. elapsed={time.perf_counter() - tic:.2f} s, "
    ...
    def init_device_graphs(self):
        ...
        logger.info(
            f"[SpanLogs] Capture {graph_backend[self.device]} begin. This can take up to several minutes. avail mem={before_mem:.2f} GB"
        )
        ...
        logger.info(
            f"[SpanLogs] Capture {graph_backend[self.device]} end. Time elapsed: {time.perf_counter() - tic:.2f} s. "
