...
class SchedulerReqTimeStats(ReqTimeStatsBase):
    ...
    def set_arrive_time(self, ts=None, real_ts=None):
        if ts is None:
            ts = time.perf_counter()
        if real_ts is None:
            real_ts = time.time()
        self.arrive_time = ts
        self.arrive_time_ts = real_ts

    def set_wait_queue_entry_time(self, ts=None, queue_size: int = 0):
        ts = ts or time.perf_counter()
        self.wait_queue_size = queue_size
    ...
    def set_prefill_bootstrap_queue_entry_time(self, ts=None, queue_size: int = 0):
        ts = ts or time.perf_counter()
    ...
    def set_decode_prealloc_queue_entry_time(self, ts=None, queue_size: int = 0):
        ts = ts or time.perf_counter()
